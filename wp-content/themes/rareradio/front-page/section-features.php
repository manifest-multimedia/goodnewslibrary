<div class="front_page_section front_page_section_features<?php
	$rareradio_scheme = rareradio_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $rareradio_scheme ) && ! rareradio_is_inherit( $rareradio_scheme ) ) {
		echo ' scheme_' . esc_attr( $rareradio_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( rareradio_get_theme_option( 'front_page_features_paddings' ) );
	if ( rareradio_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$rareradio_css      = '';
		$rareradio_bg_image = rareradio_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$rareradio_anchor_icon = rareradio_get_theme_option( 'front_page_features_anchor_icon' );
	$rareradio_anchor_text = rareradio_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( rareradio_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$rareradio_css      = '';
			$rareradio_bg_mask  = rareradio_get_theme_option( 'front_page_features_bg_mask' );
			$rareradio_bg_color_type = rareradio_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $rareradio_bg_color_type ) {
				$rareradio_bg_color = rareradio_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$rareradio_caption = rareradio_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $rareradio_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $rareradio_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $rareradio_caption, 'rareradio_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$rareradio_description = rareradio_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $rareradio_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $rareradio_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $rareradio_description ), 'rareradio_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! rareradio_exists_trx_addons() ) {
					...
					rareradio_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
