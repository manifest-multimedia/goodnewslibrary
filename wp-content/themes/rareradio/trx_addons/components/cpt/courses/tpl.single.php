    ...
    <article id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>" <?php post_class( 'courses_single itemscope' ); trx_addons_seo_snippets('', 'Article'); ?>>
		...
		<section class="courses_page_header">	
			...
			if ( !trx_addons_sc_layouts_showed('featured') && has_post_thumbnail() ) {
				...
				?>
                <div class="courses_page_featured_meta">
                    <?php
                        if (!empty($meta['time'])) {
                            ?><div class="courses_time"><?php echo esc_html($meta['time']); ?></div><?php
                        }
                    ?>                        
                    <div class="courses_date">
                        <?php
                            $dt = $meta['date'];
                            echo sprintf($dt < date('Y-m-d') ? esc_html__('%s', 'rareradio') : esc_html__('%s', 'rareradio'), '<span class="courses_item_date">' . date_i18n(get_option('date_format'), strtotime($dt)) . '</span>');
                        ?>
                    </div>
			...
			?><div class="courses_page_meta">
				<span class="courses_page_meta_item courses_page_meta_date"><?php
					$dt = $meta['date'];
					echo sprintf($dt < date('Y-m-d') ? esc_html__('Started on %s', 'rareradio') : esc_html__('Starting %s', 'rareradio'), '<span class="courses_page_meta_item_date">' . date_i18n(get_option('date_format'), strtotime($dt)) . '</span>');
				?></span><?php
				if (!empty($meta['time'])) {
		...
		if (!empty($meta['product']) && (int) $meta['product'] > 0) {
			?><div class="courses_page_buttons">
				<a href="<?php the_permalink($meta['product']); ?>" class="sc_button theme_button"><?php esc_html_e('Join the Course', 'rareradio'); ?></a>
			</div><?php
		}
