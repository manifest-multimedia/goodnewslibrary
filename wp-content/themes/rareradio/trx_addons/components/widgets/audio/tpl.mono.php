<?php
/**
 * The style "mono" of the Widget "Audio"
 *
 * @package WordPress
...
if ( is_array( $media ) && count( $media ) > 0 ) {
	...
	?><div class="trx_addons_audio_wrap<?php echo esc_attr( $wrap_class ); echo esc_attr(!empty($args['type']) ? ' sc_audio_style_'.$args['type'] : ''); ?><?php echo ( count( $media ) > 1 ) ? ' with_nav' : ''; ?>">
		...
		foreach ( $media as $item ) {
			...
			?>
				...
				">
					...
					if ( ! empty( $item['cover'] ) ) {
					?>
						<div class="img-audio"<?php	echo ' style="background-image:url(' . esc_url( $item['cover'] ) . ');"';?>></div>
					<?php
					}
					?>
					<?php
					if ( '#' !== $now_text ) {
					?>
						<div class="audio_now_playing_wrap"><?php echo '<h5 class="audio_now_playing">' . ( ! empty( $now_text ) ? esc_html( $now_text ) : esc_html__( 'Now Playing', 'rareradio' ) ) . ' <span></span></h5>';?></div>
					<?php
					}
					?>
