...
if ( ! function_exists( 'rareradio_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'rareradio_hovers_theme_setup3', 3 );
	function rareradio_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		rareradio_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'rareradio' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'rareradio' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'rareradio' ),
			
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'rareradio' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'rareradio' ) ),
					'std'      => 'icon',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'rareradio' ),
					),
					'options'  => rareradio_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'rareradio_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'rareradio_hovers_theme_setup9', 9 );
	function rareradio_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'rareradio_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'rareradio_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'rareradio_hovers_responsive_styles', 2000 );
		add_filter( 'rareradio_filter_localize_script', 'rareradio_hovers_localize_script' );
		add_filter( 'rareradio_filter_merge_scripts', 'rareradio_hovers_merge_scripts' );
		add_filter( 'rareradio_filter_merge_styles', 'rareradio_hovers_merge_styles' );
		add_filter( 'rareradio_filter_merge_styles_responsive', 'rareradio_hovers_merge_styles_responsive' );
		add_filter( 'rareradio_filter_get_css', 'rareradio_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'rareradio_hovers_frontend_scripts' ) ) {
	
	function rareradio_hovers_frontend_scripts() {
		if ( rareradio_is_on( rareradio_get_theme_option( 'debug_mode' ) ) ) {
			$rareradio_url = rareradio_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $rareradio_url ) {
				wp_enqueue_script( 'rareradio-hovers', $rareradio_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'rareradio_hovers_frontend_styles' ) ) {
	
	function rareradio_hovers_frontend_styles() {
		if ( rareradio_is_on( rareradio_get_theme_option( 'debug_mode' ) ) ) {
			$rareradio_url = rareradio_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $rareradio_url ) {
				wp_enqueue_style( 'rareradio-hovers', $rareradio_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'rareradio_hovers_responsive_styles' ) ) {
	
	function rareradio_hovers_responsive_styles() {
		if ( rareradio_is_on( rareradio_get_theme_option( 'debug_mode' ) ) ) {
			$rareradio_url = rareradio_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $rareradio_url ) {
				wp_enqueue_style( 'rareradio-hovers-responsive', $rareradio_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'rareradio_hovers_merge_styles' ) ) {
	
	function rareradio_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'rareradio_hovers_merge_styles_responsive' ) ) {
	
	function rareradio_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'rareradio_hovers_localize_script' ) ) {
	
	function rareradio_hovers_localize_script( $arr ) {
		$arr['button_hover'] = rareradio_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'rareradio_hovers_merge_scripts' ) ) {
	
	function rareradio_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'rareradio_hovers_add_icons' ) ) {
	function rareradio_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php rareradio_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
					...
					rareradio_show_layout(
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-iconmonstr-basket-2 button add_to_cart_button'
																. ' product_type_' . $product->get_type()
																. ' product_' . ( $product->is_purchasable() && $product->is_in_stock() ? 'in' : 'out' ) . '_stock'
																. ( $product->supports( 'ajax_add_to_cart' ) ? ' ajax_add_to_cart' : '' )
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( ' ', 'rareradio' ) : esc_html__( ' ', 'rareradio' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'rareradio' );
					} else {
						esc_html_e( 'Details', 'rareradio' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php rareradio_show_layout($target); ?> aria-hidden="true" class="icon-add-plus-button"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php rareradio_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php rareradio_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php rareradio_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'rareradio_hovers_get_css' ) ) {
	
	function rareradio_hovers_get_css( $css, $args ) {
