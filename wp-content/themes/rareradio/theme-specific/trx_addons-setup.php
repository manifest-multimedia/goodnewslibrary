...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'rareradio_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'rareradio_trx_addons_theme_specific_components' );
	function rareradio_trx_addons_theme_specific_components( $enable = false ) {
		return RARERADIO_THEME_FREE
					? false     // Free version
					: false;     // Pro version or Developer mode
	}
}

if ( ! function_exists( 'rareradio_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'rareradio_trx_addons_theme_specific_setup1', 1 );
	function rareradio_trx_addons_theme_specific_setup1() {
		if ( rareradio_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'rareradio_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'rareradio_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'rareradio_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'rareradio_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'rareradio_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'rareradio_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'rareradio_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'rareradio_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'rareradio_trx_addons_theme_specific_setup' );
	function rareradio_trx_addons_theme_specific_setup() {
		if ( rareradio_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'rareradio_action_before_post_meta', 'rareradio_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'rareradio_trx_addons_cv_enable' ) ) {
	
	function rareradio_trx_addons_cv_enable( $enable = false ) {
...
// Demo mode
if ( ! function_exists( 'rareradio_trx_addons_demo_enable' ) ) {
	
	function rareradio_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'rareradio_trx_addons_edd_themes_market_enable' ) ) {
	
	function rareradio_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'rareradio_trx_addons_api_list' ) ) {
	
	function rareradio_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'rareradio_trx_addons_cpt_list' ) ) {
	
	function rareradio_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'rareradio_trx_addons_sc_list' ) ) {
	
	function rareradio_trx_addons_sc_list( $list = array() ) {
		...
		$list['blogger']['templates']['default']['classic'] = array(
			'title'  => esc_html__('Classic Grid', 'rareradio'),
			'layout' => array(
				'featured' => array(
					'tr' => array(
						'meta_likes'
...
// Widgets
if ( ! function_exists( 'rareradio_trx_addons_widgets_list' ) ) {
	
	function rareradio_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'rareradio_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'rareradio_trx_addons_menu_cache' );
	function rareradio_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'rareradio_trx_addons_localize_script' ) ) {
	...
	function rareradio_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = rareradio_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'rareradio_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'rareradio_trx_addons_sc_type', 10, 2 );
	function rareradio_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = rareradio_array_merge( $list, rareradio_get_list_blog_styles( false, 'sc' ) );
		}

		if ( 'trx_sc_audio' == $sc ) {
			$list = rareradio_array_merge( $list, array (
				'default' => esc_html__('Default', 'rareradio'),
				'modern' => esc_html__('Modern', 'rareradio'),
				'simple' => esc_html__('Default White', 'rareradio'),
				'mono' => esc_html__('Mono', 'rareradio'),
			) );
		}

		// new type sc_courses
		if ($sc == 'trx_sc_courses') {
			$list['default modern'] = 'Modern';
		}
...
if ( ! function_exists( 'rareradio_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'rareradio_trx_addons_sc_prepare_atts', 10, 2 );
	function rareradio_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = rareradio_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = rareradio_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'rareradio_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'rareradio_trx_addons_sc_atts', 10, 2 );
	function rareradio_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'rareradio_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'rareradio_trx_addons_sc_map', 10, 2 );
	function rareradio_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'rareradio' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'rareradio' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( rareradio_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'rareradio' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'rareradio' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( rareradio_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'rareradio_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'rareradio_trx_addons_sc_output', 10, 4 );
	function rareradio_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! rareradio_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'rareradio_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'rareradio_trx_addons_sc_item_link_classes', 10, 3 );
	function rareradio_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! rareradio_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'rareradio_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'rareradio_trx_addons_sc_item_title_tag' );
	function rareradio_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'rareradio_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'rareradio_trx_addons_sc_item_button_args', 10, 3 );
	function rareradio_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'rareradio_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'rareradio_trx_addons_slider_title', 10, 2 );
	function rareradio_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'rareradio_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'rareradio_trx_addons_sc_googlemap_styles' );
	function rareradio_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'rareradio' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'rareradio_trx_addons_action_before_post_meta' ) ) {
	
	function rareradio_trx_addons_action_before_post_meta() {
		if ( rareradio_exists_trx_addons() ) {
...
if ( ! function_exists( 'rareradio_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'rareradio_trx_addons_tiny_mce_style_formats' );
	function rareradio_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'rareradio' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'rareradio' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'rareradio_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'rareradio_filter_allow_override_header_image', 'rareradio_trx_addons_allow_override_header_image' );
	function rareradio_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					rareradio_is_team_page()
					|| rareradio_is_cars_page()
					|| rareradio_is_cars_agents_page()
					|| rareradio_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'rareradio_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'rareradio_trx_addons_thumb_size', 10, 2 );
	function rareradio_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
if ( ! function_exists( 'rareradio_trx_addons_override_options_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'rareradio_trx_addons_override_options_fields', 10, 2 );
	function rareradio_trx_addons_override_options_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'rareradio' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'rareradio' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
