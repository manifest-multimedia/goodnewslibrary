...
jQuery( document ).ready(
	function() {
		...
		rareradio_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="rareradio_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.rareradio_post_editor' );
					if ( link.length > 0 ) {
						rareradio_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.rareradio_post_editor', function(e) {
					rareradio_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('rareradio_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function rareradio_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
